...
let
  inherit (lib)
    ...
    ;
  inherit (pkgs) stdenv;
  cfg = config.emnt.dev_env.rust;
  tomlFormat = pkgs.formats.toml { };
in
{
  options.emnt.dev_env.rust = {
    ...
    web = mkOption {
      default = true;
      description = "Wheter to enable additional tools for web Rust development.";
      example = false;
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    home.packages = mkMerge [
      ...
      (mkIf cfg.web (
        builtins.attrValues {
          inherit (pkgs) trunk cargo-leptos cargo-tauri;
        }
      ))
    ];
    ...
    home.file.".cargo/config.toml" = mkIf stdenv.isLinux {
      source = tomlFormat.generate "cargo-config" {
        target.${stdenv.targetPlatform.config} = {
          ...
          rustflags = [
            "-C"
            "link-arg=--ld-path=${pkgs.mold}/bin/mold"
          ];
        };
      };
    };
