...
let
  inherit (lib)
    ...
    hasSuffix
    ;
  cfg = config.emnt.dev_envs;

  envFilenames = attrNames (
    filterAttrs (n: v: v == "regular" && n != "default.nix" && (hasSuffix ".nix" n)) (
      builtins.readDir ./.
    )
  );
...
in
{
  ...
  config = mkIf cfg.enable {
    emnt.dev_env =
      let
        filteredEnvs = builtins.filter (
          e: !(builtins.elem e cfg.disableEnvironments)
        ) cfg.enableEnvironments;
      in
      lib.genAttrs filteredEnvs (_: {
        enable = true;
      });
  };
}
