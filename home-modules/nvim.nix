...
in
{
    ...
    emnt.nvim = {
      ...
      gitUrl = {
        fetch = mkOption {
          ...
          default = "https://github.com/eaglesemanation/nvim-config.git";
        };
        push = mkOption {
          type = types.nonEmptyStr;
          description = "Git repo URL for pushing config updates";
          default = "git@github.com:eaglesemanation/nvim-config.git";
        };
      };
  ...
  config = mkIf cfg.enable {
    ...
    home = {
      sessionVariables = mkIf cfg.setAsEditor { EDITOR = "nvim"; };
      # Pulls Neovim config from git repo and keeps it decoupled from nix conf
      activation.nvimConfGit = ''
        if [ -v DRY_RUN ]; then
